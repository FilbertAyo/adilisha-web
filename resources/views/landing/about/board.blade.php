<x-landing-layout>
    @push('head')
        @if($boards->isNotEmpty() && $boards->take(3)->filter(fn($b) => $b->profile_image)->isNotEmpty())
            @foreach($boards->take(3) as $board)
                @if($board->profile_image)
                    <link rel="preload" as="image" href="{{ asset('storage/' . $board->profile_image) }}">
                @endif
            @endforeach
        @endif
    @endpush
            ...
            <div class="col-lg-4 col-md-6 d-flex mb-4 ftco-animate">
              <div class="staff w-100">
                <div class="img mb-4" style="position: relative; overflow: hidden; aspect-ratio: 1/1; background-color: #f8f9fa;">
                  <img 
                    src="{{ $board->profile_image ? asset('storage/' . $board->profile_image) : asset('front-end/images/person_1.jpg') }}" 
                    alt="{{ $board->name }}"
                    loading="lazy"
